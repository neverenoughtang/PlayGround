...
class BGPRouting(NetworkEnvBase):
    ...
    def __init__(self, lab_name="simple_bgp"):
        ...
        # 主机 (终端节点)
        # R1 连接 1 个主机
        h1 = self.lab.add_node("h1", self.lab.images["ubuntu"], x=200, y=400)
        
        # R2 连接 2 个主机
        h2 = self.lab.add_node("h2", self.lab.images["ubuntu"], x=450, y=400)
        h3 = self.lab.add_node("h3", self.lab.images["ubuntu"], x=550, y=400)
        
        # R3 连接 1 个主机
        h4 = self.lab.add_node("h4", self.lab.images["ubuntu"], x=800, y=400)
