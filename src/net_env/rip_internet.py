...
class RIPInternet(NetworkEnvBase):
    ...
    def __init__(self, lab_name="rip_internet"):
        ...
        # 终端设备 (PC & Server)
        pc1 = self.lab.add_node("pc1", self.lab.images["ubuntu"], x=750, y=300)
        server = self.lab.add_node("server", self.lab.images["ubuntu"], x=750, y=200) # R1 旁挂服务器
        pc2 = self.lab.add_node("pc2", self.lab.images["ubuntu"], x=750, y=500)
        pc3 = self.lab.add_node("pc3", self.lab.images["ubuntu"], x=400, y=550)
        pc4 = self.lab.add_node("pc4", self.lab.images["ubuntu"], x=200, y=550)
