...
class StaticRouting(NetworkEnvBase):
    """
    静态路由场景 (规模扩容): 3 Router, 6 Switch, 12 Host (共 21 节点)
    """
    def __init__(self, lab_name="static_routing"):
        ...
        # 主机 (每个交换机下挂2台主机，共12个)
        hosts = [self.lab.add_node(f"h{i}", self.lab.images["new_ubuntu"], x=50*i, y=400) for i in range(1, 13)]
        ...
        for i in range(6):
            for j in range(2):
                self.lab.add_link(switches[i], hosts[i*2 + j])
...
if __name__ == "__main__":
    ...
    print("--- 测试连通性 (跨全网) ---")
    print(Lab_Static.lab.ping_pair("h1", "h10"))