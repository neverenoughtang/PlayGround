...
class OSPFRouting(NetworkEnvBase):
    ...
    def __init__(self, lab_name="ospf_enterprise"):
        ...
        dns = self.lab.add_node("dns", self.lab.images["ubuntu"], x=420, y=50)
        dhcp = self.lab.add_node("dhcp", self.lab.images["ubuntu"], x=500, y=50)
        lb = self.lab.add_node("lb", self.lab.images["ubuntu"], x=580, y=50)
        ...
        # 终端主机
        h1 = self.lab.add_node("h1", self.lab.images["ubuntu"], x=300, y=600)
        h2 = self.lab.add_node("h2", self.lab.images["ubuntu"], x=700, y=600)
