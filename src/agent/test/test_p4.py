...
P4_FAULT_SUITE = [
    ("inject_bmv2_process_crash", "bmv2_process_crash"),
    ("inject_p4_table_drop", "p4_table_drop"),
    ("inject_p4_wrong_forwarding", "p4_wrong_forwarding")
]
...
def run_all_link_tests():
    ...
    print(f"\n{'='*70}")
    print(f"🧪 [开始自动化测试套件]: P4 类故障 (p4 Faults)")
    print(f"📍 测试场景: {lab_name}")
    print(f"📝 包含用例: {len(P4_FAULT_SUITE)} 个")
    print(f"{'='*70}\n")
    
    # 记录整体耗时和通过情况
    start_time = time.time()
    
    for idx, (fault_name, expected_root_cause) in enumerate(P4_FAULT_SUITE, 1):
        print(f"\n" + "🌟"*35)
        print(f"▶️  [执行用例 {idx}/{len(P4_FAULT_SUITE)}]: {fault_name}")
        print("🌟"*35)
