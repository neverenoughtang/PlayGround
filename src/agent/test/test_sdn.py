...
SDN_FAULT_SUITE = [
    ("inject_sdn_controller_crash", "sdn_controller_crash"),
    ("inject_ovs_disconnect_controller", "ovs_disconnect_controller"),
    ("inject_ovs_global_drop_flow", "ovs_global_drop_flow")
]

def run_all_link_tests():
    """
    顺序运行所有 SDN 相关的网络故障智能体诊断测试
    """
    ...
    print(f"\n{'='*70}")
    print(f"🧪 [开始自动化测试套件]: SDN 类故障 (sdn Faults)")
    print(f"📍 测试场景: {lab_name}")
    print(f"📝 包含用例: {len(SDN_FAULT_SUITE)} 个")
    print(f"{'='*70}\n")
    
    # 记录整体耗时和通过情况
    start_time = time.time()
    
    for idx, (fault_name, expected_root_cause) in enumerate(SDN_FAULT_SUITE, 1):
        print(f"\n" + "🌟"*35)
        print(f"▶️  [执行用例 {idx}/{len(SDN_FAULT_SUITE)}]: {fault_name}")
        print("🌟"*35)
