...
BGP_FAULT_SUITE = [
    ("inject_bgp_neighbor_shutdown", "bgp_neighbor_shutdown"),
    ("inject_bgp_withdraw_route", "bgp_withdraw_route"),
    ("inject_bgp_wrong_peer_asn", "bgp_wrong_peer_asn")
]

def run_all_link_tests():
    """
    顺序运行所有 BGP 相关的网络故障智能体诊断测试
    """
    ...
    print(f"\n{'='*70}")
    print(f"🧪 [开始自动化测试套件]: BGP 类故障 (BGP Faults)")
    print(f"📍 测试场景: {lab_name}")
    print(f"📝 包含用例: {len(BGP_FAULT_SUITE)} 个")
    print(f"{'='*70}\n")
    
    # 记录整体耗时和通过情况
    start_time = time.time()
    
    for idx, (fault_name, expected_root_cause) in enumerate(BGP_FAULT_SUITE, 1):
        print(f"\n" + "🌟"*35)
        print(f"▶️  [执行用例 {idx}/{len(BGP_FAULT_SUITE)}]: {fault_name}")
        print("🌟"*35)
