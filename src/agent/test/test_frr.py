...
FRR_FAULT_SUITE = [
    ("inject_route_missing", "route_missing"),
    ("inject_static_route_blackhole", "static_route_blackhole"),
    ("inject_router_data_plane_drop", "router_data_plane_drop")
]

def run_all_link_tests():
    """
    顺序运行所有 FRR 相关的网络故障智能体诊断测试
    """
    ...
    print(f"\n{'='*70}")
    print(f"🧪 [开始自动化测试套件]: FRR 类故障 (Frr Faults)")
    print(f"📍 测试场景: {lab_name}")
    print(f"📝 包含用例: {len(FRR_FAULT_SUITE)} 个")
    print(f"{'='*70}\n")
    
    # 记录整体耗时和通过情况
    start_time = time.time()
    
    for idx, (fault_name, expected_root_cause) in enumerate(FRR_FAULT_SUITE, 1):
        print(f"\n" + "🌟"*35)
        print(f"▶️  [执行用例 {idx}/{len(FRR_FAULT_SUITE)}]: {fault_name}")
        print("🌟"*35)
