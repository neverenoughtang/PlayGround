...
评分标准：
1. 逻辑得分 (40分): 是否充分利用了全局巡检？Supervisor 的假设拆分是否合理？Worker 的排查轨迹是否清晰连贯？
2. 效率得分 (30分): Token 开销和工具调用数是否克制？并行机制是否显著缩短了耗时？注意拓扑规模越大、故障注入越多， token、工具调用和耗时自然会越大，需要酌情给分。
3. 准确得分 (30分): 请根据传入的 精确率(Precision) 和 正确/召回率(Recall) 严格按比例打分。
