...
async def prewarm_diagnose_caches(lab_name: str):
    ...
    """
    global _MCP_TOOLS_CACHE, _MCP_CLIENTS, _GLOBAL_EXPERIENCE_RAG_CACHE
...
async def create_worker_tools(lab_name: str):
    ...
    async def smart_mentor_tool(key: str, knowledge_count: int = 6, experience_count: int = 4) -> str:
        ...
        Args:
            key(str): 关键词
            knowledge_count(int): 返回的知识点数量，默认为 6
        ...
        sql_info = await mysql_db.search(lab_name, key, limit=experience_count)
